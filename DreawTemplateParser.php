<?php
    ...
    */
    ...
    define('FOREACH_NO_DATA', '<tr><td>No data</td></tr>');
        ...
        */
        public function parseTemplate($view = 1) {
            ...
            if (count($this->_tags) > 0 || count($this->_DBTags) > 0 || count($this->_DBCycleTags) > 0) {
                ...
                if (count($this->_DBCycleTags) > 0) {
                    foreach ($this->_DBCycleTags as $key) {
                        if (count($key) > 0) {
                            foreach ($this->_DBCycleTags as $tags => $tag) {
                                foreach ($tag as $keys => $values) {

                                    $repeat = $this->command($this->_template, '{' . $tags . '|foreach}', '{' . $tags . '|/foreach}');
                                    
                                    $this->_cache .= $repeat;

                                    foreach ($values as $key => $value) {
                                        $this->_cache = $this->commandEvaluation($key, $value, $this->_cache);
                                    }

                                    $this->_cache = $this->condition($this->_cache);
                                    $this->_cache = $this->numInc($this->_cache);
                                }

                                $this->_template = str_replace('{' . $tags . '|foreach}' . $repeat . '{' . $tags . '|/foreach}', $this->_cache, $this->_template);
                                $this->_cache = null;
                            }
                        }

                        else {
                            foreach ($this->_DBCycleTags as $key => $value) {
                                $repeat = $this->command($this->_template, '{' . $key . '|foreach}', '{' . $key . '|/foreach}');
                                $this->_template = $this->str_replace_first('{' . $key . '|foreach}' . $repeat . '{' . $key . '|/foreach}', 'lol', $this->_template);
                            }
                        }
                    }
                }
        ...
        protected function getToken() {
            if (strstr($this->_template, '{getToken}')) {
                $sess = new DreawSessHandler();
                $sess->setSess('token', sha1(uniqid($sess->getSess('name'), true)));

                session_regenerate_id();

                $this->_template = str_replace('{getToken}', "<input type='hidden' name='token' value='{$sess->getSess('token')}' />", $this->_template);
            }
        ...
        */
        ...
        protected function condition($source = null) {
            ...
            if (strstr($source, '{endif}')) { // ale to jen tak O :D Kamčo, já jsem rád, že jsi to udělal geniálně, akorát ostatní je v piči :D :D :D Dělám si srandu :D :D No, každopádně, mě volají naši ať jdu za nima, takže tě tu chvíli neechám a pak mi napíšeš na FB jak budeš cokoliv potřebovat nebo bude změna statusu? :D .. hměna jak statusu? změna stavu z V PIči NA funkční ... jasné, řeknu, teď jen pudu na WC, hned jsem tu :D Dobře :) jasan a ahooj :D zatím :D
                $exploded_condition = explode('{endif}', $source); // musím jít papat, za chvíli jsem tu ;) [18:13] [18:23] - jsem tady
        ...
        protected function UrgencyNormal($urgency) {
            if ($urgency == '1') {
                return true;
            }
        ...
        protected function UrgencyImportant($urgency) {
            if ($urgency == '2') {
                return true;
            }
        ...
        protected function UrgencyCritical($urgency) {
            if ($urgency == '3') {
                return true;
            }
